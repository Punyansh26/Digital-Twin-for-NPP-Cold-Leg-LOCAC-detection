...
\added{Retained accuracy evidence} & \added{Pooled $R^2$: 0.9951--0.9962} & \added{Matched-scale case-0 panel} & \added{Matched-scale case-0 panel} \\
...
\begin{itemize}
    ...
    \item \added{\textbf{The Transolver-inspired operator} supplies global token attention and has 3.245M parameters.}
...
\added{\textbf{The Transolver-inspired operator} provides global interactions over $M=64$ tokens, with $\mathcal{O}(M^2)$ token-attention complexity, and is the largest studied model at 3.245M parameters. Because all training and evaluation use the same mesh, transfer across topology is a hypothesis rather than a result.}
...
\textbf{Run-to-run stability}: Operator results and the primary classifier artifact are single-run. Only the constrained translation-path ablation has five seeds. \added{Future comparisons will use predeclared seeds, immutable run manifests, and mean $\pm$ sample standard deviation under both current and matched-objective protocols.}
...
\begin{itemize}
    ...
    \item \added{\textbf{The Transolver-inspired operator} has 3,244,872 parameters and provides global attention over 64 learned mesh tokens, offering a distinct mechanism for long-range spatial interaction within the unified interface.}
