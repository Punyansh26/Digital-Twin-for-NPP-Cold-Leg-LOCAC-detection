# AGENTS.md — AP1000 Digital Twin for LOCAC Detection

> Project guidance for coding agents working in this repository. Read it fully
> before making changes. `Journal/AGENTS.md` adds binding paper-editing rules
> for work under `Journal/`.
...
### 1. Preserve the scientific scope

- Treat this as a proof-of-concept research prototype, not a plant-operational
  or safety-certified system. Do not imply regulatory approval, deployment
  readiness, or validated safety performance.
- State the provenance of every result: **synthetic mock data**, **Fluent CFD**,
  **NPPAD data**, or another explicitly named source. Do not generalize a
  synthetic-benchmark result to real transients or plant operation.
- Distinguish a classifier probability from predictive uncertainty. A LOCA
  probability is a model score; it is not a confidence interval or a calibrated
  uncertainty bound unless that has been independently demonstrated.

### 2. Use an evidence hierarchy for technical claims

Before describing a feature as available, implemented in the pipeline, or
evaluated, inspect the relevant configuration, source, checkpoint, result
artifact, and invocation path. Use this hierarchy, strongest first:

1. Reproducible result artifact with the script, configuration, data split, and
   reported metric.
2. Saved checkpoint plus an executable evaluation path and recorded output.
3. Executable source code and a configuration that enables it.
4. Source-code stubs, comments, documentation, or CLI options alone.

Items in category 4 are prototypes or planned work, not evaluated capabilities.
Never manufacture metrics, infer them from a validation loss, or upgrade a
prototype to an experimental result. When evidence is incomplete, say what was
checked and mark the result `[VERIFY: ...]` rather than guessing.

### 3. Tier-3 extensions require special care

- Mamba, Liquid NN, residual multi-fidelity, wall-shear, and DDPM modules have
  separate implementation paths; their presence in `src/` or a CLI option does
  not mean that they participated in the reported benchmark.
- The default `configs/model_config.yaml` disables diffusion. A DDPM UQ claim
  requires, at minimum, a trained diffusion checkpoint, a reproducible sampling
  protocol, held-out evaluation, and uncertainty metrics such as nominal level,
  empirical coverage, interval width, and calibration. Until then, describe UQ
  as unimplemented future work in the evaluated pipeline.
- In the current inference flow, raw `turbulence_samples` are optional outputs;
  do not claim decision-level UQ unless uncertainty is propagated through
  feature translation and LOCA classification and evaluated end to end.

### 4. Metrics and comparisons

- Record exact values with the metric definition, split, seed(s), hardware, and
  model/checkpoint identity. A checkpoint filename is not sufficient evidence
  of a result.
- For multi-architecture comparisons, disclose different losses, targets, and
  optimization settings. Do not characterize the comparison as fully matched
  when DeepONetFourier uses Sobolev/divergence terms and another model uses MSE
  only.
- Keep field-reconstruction evidence separate from detection evidence. In
  particular, the feature translator blends input and CFD-derived severity;
  only a documented field-only ablation can support a claim that field
  predictions independently improve LOCA detection.
- Retain the exact benchmark scope beside speedup, accuracy, calibration, and
  reliability claims. For latency, report the timing boundary and hardware.

### 5. Reproducible execution and artifact handling

- Run project Python through `conda run -n minor_proj python ...`; do not rely
  on a system interpreter for experiments or metric extraction.
- Before an expensive run, inspect the active config, checkpoint path, dataset
  availability, GPU/CPU device, expected output location, and free disk space.
  Use a small smoke test when practical.
- Do not overwrite a checkpoint, result, figure, dataset, or user-authored
  change without explicit authorization. Existing changes may be intentional.
  Give new artifacts distinct, descriptive names and record the producing
  command and configuration.
- Treat `data/` and `results/` as local artifacts that may be absent or
  git-ignored. Their absence is evidence that a result cannot be verified; it
  is not permission to fabricate it.

### 6. Change and review discipline

- Prefer focused patches. Inspect the relevant call chain before changing model
  interfaces, tensor shapes, scalers, field ordering, configuration keys, or
  checkpoint format.
- Update documentation and CLI/config registration whenever a new operator is
  made runnable. A module is not integrated until training, inference,
  configuration, and artifact paths agree.
- Run the narrowest meaningful test or smoke check after a change and report
  what was verified, what was not run, and why. Keep generated experiment
  outputs separate from source edits.
